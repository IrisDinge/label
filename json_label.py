'''
将label中标注的json文件，转化为可用于分割训练的标签二值化png图片
'''
import os
import cv2
import numpy as np
    ...
    for eachfile in os.listdir(path_json_to_data):
        ...
        if os.path.isdir(path1):  # 判断path1路径是否存在
            if os.path.exists(path1 + '/label.png'):  # 判断path1路径下label.png是否存在
                ...
                path1 = os.path.join(path1, 'label.png')  # 获取PNG所在的路径，

                img_name = os.path.splitext(eachfile)[0]

                final_name = img_name.rpartition('_json')[0] + ".png"


                id = os.path.splitext("_")[0] + os.path.splitext("_")[1]
                path2 = os.path.join(path_save_png, final_name)  # 将png复制到path2路径下的文件夹中去
...
if __name__ == "__main__":
    ...
    path_json = r''  # 这里是指.json文件所在文件夹的路径
    # 批量转换，修改此路径
    # 此路径为，json文件所在路径
    # def extract_png():  第二次转换用到
    path_json_to_data = os.path.join(path_json, "json_data")  # json文件夹所在位置
    path_save_png = os.path.join(path_json, "annotation")  # 将标签图从json文件中批量取出后指定保存的文件目录
